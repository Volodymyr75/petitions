...
- Додано скрипт `etl/fix_motherduck_schema.py`, який створює необхідні PRIMARY KEY у `votes_history` (композитний PK) та `daily_stats` (PK на `date`).
- Скрипт можна запустити один раз після виправлення схеми.
