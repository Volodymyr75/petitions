...
## 📅 Сесія 2026-01-04: Уніфікація статусів та ріст бази
1. **База даних**: Кількість петицій зросла до **88,224** після чергового етапу backfill (діапазон до 170,000).
2. **Fix Status Logic**: Оновлено `scraper_detail.py` для підтримки нових статусів:
   - "Очікує на розгляд" → мапиться на **"На розгляді"**.
   - "Збір підписів завершено" → мапиться на **"Архів"**.
3. **Healing**: Запущено скрипт `fix_unknowns.py`, який виправив 8 застарілих статусів "Unknown". Тепер аналітика статусів на 100% коректна.
4. **Deploy**: Дані синхронізовано з GitHub.
